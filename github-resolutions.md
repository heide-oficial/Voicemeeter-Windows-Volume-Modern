...
Thanks for contributing to the project. This fix is included in the release v1.2.0.
...
The native C++ host was evaluated, but it would introduce a second runtime architecture, duplicated synchronization logic, and additional lifecycle and packaging complexity. The proposed host was therefore not merged. The underlying memory-growth problem was resolved in the existing managed service by reusing the attached Core Audio endpoint and limiting endpoint recreation to invalid or unavailable states.
